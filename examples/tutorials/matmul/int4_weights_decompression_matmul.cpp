...
// [K, N] => [N, K]
void transpose_s4(const std::vector<int32_t> &src,
...
void print_int4_matrix(const std::vector<int32_t> &vec, int32_t size) {
    for (int32_t i = 0; i < size; ++i) {
        int32_t byte_idx = i / 8;
        int32_t nibble_offset = i % 8;
        int8_t int4 = (vec[byte_idx] >> (nibble_offset * 4)) & 0x0F;
        std::cout << std::hex << (int)int4 << " ";
    }
    std::cout << std::endl;
}
...
void prepare_input(memory &A_f16_mem, memory &sc_B_mem, memory &zp_B_mem) {
    ...
    std::vector<float> A_f16(M * K);
    init_vector(A_f16);
    // Fill A_f16_mem f16 data from A_f16 data filled as f16
    write_to_dnnl_memory(A_f16.data(), A_f16_mem);

    std::vector<float> sc_B(NUM_G_SC * N);
    init_vector(sc_B);
    // Fill sc_B_mem f16 data from sc_B data filled as 16f
...
void infer(const matmul &matmul_p, int64_t M, int64_t N, int64_t K,
    ...
    for (int32_t run = 0; run < number_of_runs; ++run) {
        matmul_p.execute(s,
                {{DNNL_ARG_SRC, A_f16_mem},
                 {DNNL_ARG_WEIGHTS, B_s4_mem},
