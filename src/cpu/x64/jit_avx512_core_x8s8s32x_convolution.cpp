...
status_t jit_avx512_core_x8s8s32x_convolution_fwd_t::execute_forward_3d_dw(
    ...
    parallel_nd(jcp.mb, jcp.od, jcp.oh, jcp.nb_ow, nb_groups,
            [= COMPAT_THIS_CAPTURE](dim_t n, dim_t od_s, dim_t oh_s, dim_t owb,
                    dim_t gg) {
        auto p = jit_conv_args_t();
        ...
        int dilate_d = jcp.dilate_d + 1;
        int i_f_overflow
                = nstl::min(jcp.kd, div_up(nstl::max(0, -id_s), dilate_d));
        ...
        size_t wei_d_stride = (jcp.signed_input || jcp.src_zero_point)
                ? 0
        ...
        size_t wei_h_stride = (jcp.signed_input || jcp.src_zero_point)
                ? 0
                : i_t_overflow * wht_h_stride;
        p.src = src_w + i_t_overflow * dilate_h * src_h_stride
