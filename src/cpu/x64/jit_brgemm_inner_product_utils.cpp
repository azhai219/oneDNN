...
jit_brgemm_ip_conf_t::get_desired_weights_tag() const {
    ...
    if (is_not_vnni_tag ||
        (jbgp.weights_decompression && (jbgp.orig_wei_dt == u8 || jbgp.orig_wei_dt == s8 || is_vcvtph2ps_kernel) && !jbgp.with_src_dynamic_quant)) {
...
>>>>>>> dd44ac9fd1 ([FORK][FEATURE] InnerProduct primitive: squashed weight decompression)
}
