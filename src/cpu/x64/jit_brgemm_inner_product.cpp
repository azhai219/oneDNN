        ...
        if (gemm_batch > 0 && brg_kernel != nullptr) {
            ...
            for (int b = 0; b < gemm_batch; b++) {
                ...
                if (jbgp.weights_compressed) {
                    ...
                    int typesize_scale = one_of(jbgp.orig_wei_dt, data_type::s4, data_type::u4) ? 2 : 1;
                    ...
                    const int ic_internal_block = pd()->jbgp_.wei_dt == data_type::bf16 ||
                                                  one_of(pd()->jbgp_.orig_wei_dt, data_type::s4, data_type::u4) ? 2 : 1;
                    ...
                    int typesize_scale = one_of(jbgp.wei_dt, data_type::s4, data_type::u4) ? 2 : 1;
            ...
            if (jbgp.nthr_ic_b == 1 && are_post_ops_applicable
                ...
                brgemm_kernel_execute_postops(brg_kernel, gemm_batch,
                        ...
                        scratch, nullptr, (float*)wei_scales + wei_scales_offset, wei_zero_points + wei_zero_points_offset, nullptr, ic);
            } else {
                brgemm_kernel_execute(brg_kernel, gemm_batch, addr_batch,
                        (void *)ptr_C, is_amx ? (void *)wsp_tile : nullptr, nullptr, (float*)wei_scales + wei_scales_offset, wei_zero_points + wei_zero_points_offset, nullptr, ic);
            }
        }
        ...
        if (is_ic_tail) {
            ...
            if (jbgp.nthr_ic_b == 1 && are_post_ops_applicable
                ...
                brgemm_kernel_execute_postops(brg_kernel_ic_tail, 1, addr_batch,
                        (void *)ptr_C, (void *)ptr_D, post_ops_data, scratch, nullptr, (float*)wei_scales + wei_scales_offset, wei_zero_points + wei_zero_points_offset, nullptr, ic);
            } else {
