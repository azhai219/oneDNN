...
status_t ip_attr_check(const inner_product_desc_t &desc, const engine_t *engine,
    ...
    bool is_weight_compression = (one_of(src_dt, data_type::f32, data_type::bf16) && one_of(wei_dt, data_type::u8, data_type::s8, data_type::s4, data_type::u4)) ||
    ...
    if (utils::one_of(desc.prop_kind, prop_kind::forward_inference,
        ...
        if (engine->kind() == engine_kind::cpu)
            is_int8 |= one_of(wei_dt, data_type::u8, data_type::s8, data_type::s4, data_type::u4);
